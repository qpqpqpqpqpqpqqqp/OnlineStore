...
router
    .route('/')
    .get((req, res) => res.render(path.resolve('public/html_files/access.ejs'), {ans: 1, holder: 'USD'}))
    .post((req, res) => {
        ...
        try{
            https.get(url, function (response) {
                response.on('data', d => {
                    ...
                    let curr = data.rate;
                    res.render(path.resolve('public/html_files/access.ejs'), {ans: curr, holder: to});
                })
            });
        } catch (e) {
            console.log("error!")
